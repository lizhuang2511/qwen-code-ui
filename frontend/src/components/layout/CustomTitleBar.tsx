...
import { getPlatform, isPywebview } from "@/lib/runtime";
...
export const CustomTitleBar: React.FC<CustomTitleBarProps> = ({
  ...
  const [shouldShow, setShouldShow] = useState<boolean>(__WEB__);
  useEffect(() => {
    if (__WEB__) {
      setShouldShow(true);
      return;
    ...
    const detect = async () => {
      ...
      setShouldShow(!isPywebview() && p === "windows");
    };
    detect();
  }, []);
          ...
          >
            ...
            >
              ...
              <DropdownMenuItem
                onClick={() => {
                  setOpenMenu(null);
                  window.dispatchEvent(new CustomEvent("app:open-web-settings"));
                }}
                className="flex items-center justify-between text-xs"
              >
                <div className="flex items-center gap-2">
