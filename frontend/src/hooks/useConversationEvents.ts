    ...
    async (conversationId: string): Promise<() => void> => {
      ...
      try {
        const unlistenCliIo = await listen<{
          ...
          console.log("🐛 [cli-io]", sanitizedId, event.payload.type, event.payload.data);
          setCliIOLogs((prev) => [
            ...prev,
            {
              timestamp: new Date(),
          ...
          try {
            ...
            if (event.payload.type === "output") {
              ...
              // If it's a requestToolCallConfirmation or session/request_permission input, store it for when the tool call is created
              if (
