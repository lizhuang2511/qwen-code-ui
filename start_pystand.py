...
def start_backend():
    ...
    # Return an object that mimics subprocess.Popen interface for cleanup
    class ServerHandle:
        def terminate(self):
            server.should_exit = True
            
        def wait(self):
            # In a thread model, we might not want to block indefinitely if the server is stuck,
            # but for this interface, join is appropriate.
            # However, since daemon=True, it will die with main thread.
            # But cleanup calls wait(), so we should allow it to return.
            if t.is_alive():
                t.join(timeout=5.0)
